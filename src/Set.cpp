 ...
 *   sets.
 */
void Set::operatorUnion(const Set& otherSet)
{
  for (int i = 0; i < otherSet.setSize; i++)
  {
    addItem(otherSet.setItem[i]);
  }
}
 ...
 *   the two sets.
 */
void Set::operatorIntersect(const Set& otherSet)
{
  for (int i = setSize - 1; i >= 0; i--)
  {
    if (otherSet.containsItem(setItem[i]) == false)
    {
